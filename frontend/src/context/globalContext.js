...
const BASE_URL = "https://cashtracker.onrender.com/api/v1/";
...
export const GlobalProvider = ({ children }) => {
  ...
  const getCategories = async () => {
    try {
      ...
      const customIncomeCats = incomeCats.filter(
        (category) => !defaultIncomeCategories.includes(category.name)
      );
      const customExpenseCats = expenseCats.filter(
  ...
  const deleteCategory = async (id) => {
    ...
    for (const category of customIncomeCategories.concat(
